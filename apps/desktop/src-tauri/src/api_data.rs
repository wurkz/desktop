...
pub async fn sync_changes(
    ...
    let specs: [(&str, &str); 14] = [
        ...
        ("suppliers", "updated_at"),            // protocol v1.1: supplier master records
    ];
    let mut tables = serde_json::Map::new();
    let mut count = 0usize;

    // Protocol v1.2 (BACK-4-009): shop-settings snapshot — a single curated row from app_config
    // (formatting + policy context for cloud analytics, e.g. discount-cap breach detection).
    // Explicit column list: NEVER the whole row — device_token/cloud_url must not leave the shop.
    if let Ok(rows) = sqlx::query(
        "SELECT id, shop_name, currency_symbol, tax_rate, vat_status, tax_inclusive, \
                max_discount_pct, updated_at \
         FROM app_config WHERE updated_at > ?",
    )
    .bind(since)
    .fetch_all(&state.pool)
    .await
