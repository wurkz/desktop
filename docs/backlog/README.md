...
| Phase 3 — Commerce Module | [backlog](./phase-3-backlog.md) | [completed](./phase-3-completed.md) | ~75% (**inventory management page + stock adjustments + CSV imports (parts & customers) + stock deduction** done; 001 superseded by D23; **tax config (008)** done; **payment methods (007)** + **VAT-inclusive pricing (009)** *(implemented, pending verify)*; nothing left except deferred) |
| Phase 4 — Cloud Link | [backlog](./phase-4-backlog.md) | [completed](./phase-4-completed.md) | ~10% (backend parked; **client-side cloud-link prep shipped**: unique tenant identity + cloud config + Cloud Link settings, all opt-in/off by default. 8 backend items remain) |
