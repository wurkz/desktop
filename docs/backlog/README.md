...
| Phase 1 — Core Kernel | [backlog](./phase-1-backlog.md) | [completed](./phase-1-completed.md) | ~88% (settings, **data-driven asset types**, **demo seeder/reset**, **schema domain split** done; BACK-1-002 module registry **obsolete** (superseded by D23), **user-mgmt UI (003) delivered by BACK-0-007**; remaining all deferred: hardware IO) |
| Phase 2 — Repair Module | [backlog](./phase-2-backlog.md) | [completed](./phase-2-completed.md) | ~98% (everything shipped; 14 items **implemented, all pending the batched final-QA verify**: **BACK-2-015** mechanic dashboard cleanup + backup API gating, **BACK-2-016** Start Job role logic + on-behalf start, **BACK-2-017** ticket back-navigation *(implemented, pending verify)*, **BACK-2-018** Appearance → Settings *(implemented, pending verify)*, **BACK-2-019** mobile one-row KPI strip *(implemented, pending verify)*, **BACK-2-020** billing actions staff-only *(implemented, pending verify)*, **BACK-2-021** case-insensitive usernames *(implemented, pending verify)*, **BACK-2-022** slide-to-confirm on actions *(implemented, pending verify)*, **BACK-2-023** assign staff-only *(implemented, pending verify)*, **BACK-2-024** responsive estimate dialog *(implemented, pending verify)*, **BACK-2-025** dyslexia-friendly mode *(implemented, pending verify)*, **BACK-2-026** de-emphasize trial banner *(implemented, pending verify)*, **BACK-2-027** QR-code login + printable QR *(implemented, pending verify)*, **BACK-2-028** mechanic checklist completion UX *(implemented, pending verify)*, **BACK-2-029** Appearance: color swatches fix + dyslexia tint modes *(implemented, pending verify)*) |
| Phase 3 — Commerce Module | [backlog](./phase-3-backlog.md) | [completed](./phase-3-completed.md) | ~75% (**inventory management page + stock adjustments + CSV imports (parts & customers) + stock deduction** done; 001 superseded by D23; **tax config (008)** done; **payment methods (007)** + **VAT-inclusive pricing (009)** *(implemented, pending verify)*; **BACK-3-020** full customer management page + CSV imports (customers & suppliers) w/ skipped-duplicates report *(implemented 2026-07-10, pending verify)*) |
| Phase 4 — Cloud Link | [backlog](./phase-4-backlog.md) | [completed](./phase-4-completed.md) | ~75% (**v1 cloud delivered 2026-07-08** — Laravel backend in [`wurkz/cloud`](https://github.com/wurkz/cloud): sync API + device tokens + owner/admin dashboards w/ money gate + platform admin/kill-switch, verified E2E against the real desktop. 001/002 superseded (Laravel/MySQL); remaining: Hostinger deploy, **BACK-4-007** customer portal, **BACK-4-008** encrypted sync) |
